...
- Selfhosted provider DOES forward `language_code` to its upstream `language` field at the API layer (the local TTS request type already has a `Language` field). However, the browser UI Language picker is populated only from `Provider.ListModels`, and selfhosted's `ListModels` returns `nil` (its models == voices upstream — populating both would duplicate the dropdowns and let users submit contradictory voice/model combos that `Synthesize` silently resolves in favor of `model_id`). Net effect: selfhosted users can use `language_code` via the API, but the UI Language dropdown is empty for selfhosted providers. Earlier fix iterations attempted to derive languages from voices for the UI, but for ElevenLabs `voice.language` is freeform metadata (e.g. `"english"`) rather than ISO 639-1, so the picker would offer codes the API rejects with 503 — the fallback was reverted.
